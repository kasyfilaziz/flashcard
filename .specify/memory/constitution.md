...
### V. Data Portability
Users MUST be able to export their **user-generated content** (flashcards, decks, or equivalent app-specific data) in standard formats (JSON, CSV). Import functionality MUST support these same formats with validation.

**Scope Clarification**: This principle applies to user-generated content that the app requires to function (e.g., flashcard decks and cards). It does NOT apply to usage statistics, session history, or telemetry data generated by the app (e.g., Pomodoro session records, study streaks, analytics). Usage statistics are considered transient operational data.

**Rationale**: Ensures users can backup, migrate, or leave the platform with their original content.
...
- Data export/import MUST be tested for each workout type that has user-generated content (flashcard app)
...
**Version**: 1.1.0 | **Ratified**: 2026-03-17 | **Last Amended**: 2026-03-19
...
**Modified Principles**:
- Principle V (Data Portability): Added scope clarification distinguishing user-generated content from usage statistics

**Key Changes**:
- Added "Scope Clarification" paragraph to Principle V
- Clarified that usage statistics (e.g., Pomodoro sessions, streaks) are NOT in scope for data portability
- Updated Compliance Review to reference only "workout types that have user-generated content"

**Rationale for Version Bump**: Material clarification that expands guidance scope without breaking existing functionality.

**Files Requiring Updates**: None - this clarification aligns with existing implementations (Flashcard app has export, Pomodoro app does not).
